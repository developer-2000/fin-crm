...
class TranslationController extends BaseController
{
    ...
    public function getIndex($group = null)
    {
        ...
        if ($group) {
            if(!isset($this->permissions['translation_group'])) {
                abort(404);
            }
            $view = 'group';
        }

        return view('translation-manager::' . $view)
            ...
            ->with('editUrl', route('translation-post-edit', [$group]))
            ->with('deleteEnabled', $this->manager->getConfig('delete_enabled'))
            ->with('langs', TranslationRepository::getLanguages());
    }
    ...
    {
        app()->setLocale('en');//en - language for new translations

        $numFound = $this->manager->findTranslations();

        event(new TranslationCreatedEvent());
    ...
    public function postAddGroup(Request $request)
    {
        ...
        if ($group)
        {
            return redirect()->action('App\Http\Controllers\TranslationController@getView',$group);
        }
        else
        {
            return redirect()->back();
