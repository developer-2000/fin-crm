...
class GetCalls extends Command
{
    ...
    public function handle(
        ...
        try {
            $status = $variablesModel->getVariable('cron_get_calls');
            if (!$status || $status->value == 1) {
                echo date('H:i:s d/m/y', time()) . " already started\n";
                exit;
            }
            $variablesModel->setVariable('cron_get_calls', 1);
...
//            $orderModel->addCallsInElastix();
            $variablesModel->setVariable('cron_get_calls', 0);
            Log::info('Calls end');
        } catch (\Exception $exception) {
            $variablesModel->setVariable('cron_get_calls', 0);
