...
class UpdateTagsAndOrders1 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'update_tags_1';
    ...
    public function handle()
    {
        ...
        $ordersArray = Order::whereBetween('time_created', ['2018-09-01 00:00:00', '2018-12-31 17:59:59'])
            ...
            ->where('entity', 'order')
            ->where('partner_id', 1)
            ->whereNULL('tag_medium')
            ->whereNULL('tag_campaign')
            ->whereNULL('tag_term')
            ->whereNULL('tag_content')
          //  ->where('partner_oid', 2316388)
        ;
        $ordersArray->chunk(300, function ( $orders ) use ( $client ) {
            $response = [];
            try {
                $response = $client->request('POST', 'http://my.sevensystem.pro/api/v2/get_tags_multi',
                    [
                        'form_params' => [
                ...
                if ($response) {
                    if ($response->getStatusCode() == 200) {
                        Storage::append('bs7-response2.log', count($orders->pluck('partner_oid')->toArray()));
                        $results = json_decode($response->getBody()->getContents(), true);
                        Storage::append('bs7-response2.log', json_encode($results));

                        Storage::append('bs7-response2.log', count($results['data']));
                        var_dump($results);
...
//                                    }
                            ...
                            var_dump(array_diff($orders->pluck('partner_oid')->toArray(), $ordersIds));
                            Storage::append('bs7-response2.log', "---diff---". json_encode(array_diff($orders->pluck('partner_oid')->toArray(), $ordersIds)));
                        }
                    }
                }
