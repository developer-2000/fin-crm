...
class MeasoftTrack extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'measoftTrack';
    ...
    {
        try {
            $status = $variablesModel->getVariable('cron_measoft_track');
            if (!$status || $status->value == 1) {
                echo date('H:i:s d/m/y', time()) . " already started\n";
                exit;
            }
            $variablesModel->setVariable('cron_measoft_track', 1);
            echo date('Y-m-d H:i:s', time()) . " - start \n";

            Measoft::track();

            $variablesModel->setVariable('cron_measoft_track', 0);
