$(function () {
     ...
     */
    getColdCallsOrdersCountModeration();
// addEditable();
    getColdCallsOrdersCountModeration(function () {
        getCountModeration();
    }, 10000);
     ...
     * модерируем заказы
     */
    $('.cold-calls-moderation').on('click', coldCallsModeration);
    $('.cold-calls-moderation').tooltip();

    function coldCallsModeration() {
        var orderId = $(this).attr('data-id');
        var parent = $(this).parents('.one_order');
        var storage = null;
        if (parent.find('select[name="sub_project_id"]').length) {
            sender = parent.find('select[name="sub_project_id"]').val();
        }
        $.post('/cold-calls/cold-calls-moderation-order-ajax/' + orderId, {sender: sender}, function (json) {
            if (json.success) {
                parent.fadeOut(600);
                setTimeout(function () {
                    parent.remove();
    ...
    function getColdCallsOrdersCountModeration() {
        var search = location.search.replace('?', '');
        $.post('/ajax/count-coldcalls-orders-on-moderation', search, function (json) {
...
function changePhone() {
    ...
    $.post('/ajax/moderation-change-phone-and-country/', {
        id: id,
        phone: phone,
        country: country,
        price: price
...
function moderation() {
    ...
    $.post('/moderation-order-ajax/' + orderId, {storage: storage}, function (json) {
        if (json.success) {
            parent.fadeOut(600);
            setTimeout(function () {
                parent.remove();
