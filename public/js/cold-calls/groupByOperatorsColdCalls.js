...
function addOperators(event, ui) {
    ...
    if (!$('.operators #' + operator).length) {
        ...
        $.post('/change-operator-queues-elastix', {queues : groupId, id : operatorHtml.attr('data-id')}, function (json) {
            if (json.status) {
                if (groupId == 0) {
...
function searchOperators(e) {
    if ((e.which >= 48 && e.which <= 90) || (e.which >= 186 && e.which <= 220) || (e.which >= 96 && e.which <= 111) || e.which == 8 || e.which == 46) {
        setTimeout(function() {
            ...
            $.post('/search-operators-for-pgx-campaign-ajax-cold-calls', {search: search}, function(json) {
                $('.wrapper').empty();
