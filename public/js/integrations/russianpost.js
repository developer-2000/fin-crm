...
function addKazSenders() {
    ...
    $.post('/ajax/integrations/russianpost/add-sender', $(this).serialize(), function (json) {
        if (json.success) {
...
function editKazSenders() {
    ...
    $.post('/ajax/integrations/russianpost/edit-sender/' + $('#sender_id').val(), form.serialize(), function (json) {
        if (json.success) {
